...
/** Integer hash -> 4 uniform floats in [0, 1). Stable per grain. */
...
vec4 grainRandom(ivec2 texel) {
  uvec4 h = pcg4d(uvec4(uint(texel.x), uint(texel.y), 0x5eedu, 0xa11ceu));
...
// Divergence-free 2D field: the curl of a scalar noise potential.
